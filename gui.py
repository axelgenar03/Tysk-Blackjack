...
class GUI:
    ...
    def cardidxchange(self,window,i):
            if self.cardidx[i] == False:
                self.cardidx[i] = True
                Label(window,text="!", fg="#FFFFFF",font=("Arial",25)).place(x=107+140*i, y=740)
            elif self.cardidx[i] == True:
                self.cardidx[i] = False
                Label(window,text="!", fg="#ECECEC",font=("Arial",25)).place(x=107+140*i, y=740)
            print(self.cardidx)
            return

    def placebuttons(self,window):
        change_card1 = Button(window, text="change" , command=lambda: self.cardidxchange(window,0))
        change_card2 = Button(window, text="change" , command=lambda: self.cardidxchange(window,1))
        change_card3 = Button(window, text="change" , command=lambda: self.cardidxchange(window,2))
        change_card4 = Button(window, text="change" , command=lambda: self.cardidxchange(window,3))
        change_card5 = Button(window, text="change" , command=lambda: self.cardidxchange(window,4))
